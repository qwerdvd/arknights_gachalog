...
async def check_db():
    ...
    for row in c_data:
        try:
            ...
            return_str = return_str + f"uid{row[0]}/{ark_id}的Cookies是正常的！\n"
            normal_num += 1
...
async def refresh_ck(uid, mysid):
    ...
    c = conn.cursor()
    try:
        c.execute("DELETE from CookiesCache where uid=?", (uid, mysid))
    except Exception:
        pass
...
async def check_token_db():
    ...
    for row in c_data:
        ...
        if "err" in data["message"] or data["statusCode"] == -401:
            invalid_str = invalid_str + f"uid{row[0]}的token是异常的！已删除该条token！\n"
