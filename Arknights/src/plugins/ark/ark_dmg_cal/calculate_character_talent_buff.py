    ...
    for i in range(len(character_talent_info)):
        ...
        if is_uniequip:
            if uniequip_id == "一模":
                ...
                uniequip_id = 2
            override_talent_data = await get_uniequip_talent_adjustment(
                characterId, uniequip_id, target
            )
            ...
            # 计算潜能对天赋加成的影响
            raw_talent_without_potential = character_talent_info[
                f"{override_talent_index + 1}"
            ]["candidates"][0]
            raw_talent_with_potential = character_talent_info[
                f"{override_talent_index + 1}"
            ]["candidates"][1]
            ...
            for item in with_potential:
                ...
                for item2 in without_potential:
                    if key == item2["key"]:
                        ...
                        potential_add.append(
                            {"key": item["key"], "value": raw_potential_add}
                        )
